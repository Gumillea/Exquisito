    ...
    public static void modifyFoods() {
        ...
        if (ExquisitoConfig.Common.FOOD_MODIFICATION.get()) {
            ...
            if (ModList.get().isLoaded(ModCompat.EED) && (ExquisitoConfig.Common.ENLIGHTEND_MODIFICATION.get())) {
                chorusFoods.add(Objects.requireNonNull(ForgeRegistries.ITEMS.getValue(new ResourceLocation(ModCompat.EED, "chorus_soup"))).foodProperties);

                Objects.requireNonNull(Objects.requireNonNull(ForgeRegistries.ITEMS.getValue(new ResourceLocation(ModCompat.EED, "jelly_ring_item")))
                        .foodProperties).effects.add(Pair.of(() -> new MobEffectInstance(fg, 200), 1.0F));
                Objects.requireNonNull(Objects.requireNonNull(ForgeRegistries.ITEMS.getValue(new ResourceLocation(ModCompat.EED, "ennegel_pasta")))
                        .foodProperties).effects.add(Pair.of(() -> new MobEffectInstance(fg, 600), 1.0F));
                Objects.requireNonNull(Objects.requireNonNull(ForgeRegistries.ITEMS.getValue(new ResourceLocation(ModCompat.EED, "zure_berry")))
                        .foodProperties).effects.add(Pair.of(() -> new MobEffectInstance(ExquisitoEffects.SPACE_DIVING.get(), 600), 1.0F));
            }
